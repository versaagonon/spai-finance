...
@section('title', 'New Disbursement Entry')

@section('content')
<div class="max-w-4xl mx-auto bg-white rounded-xl shadow-sm border border-gray-100 p-8">
    <h2 class="text-xl font-bold text-gray-800 mb-6">Input Pengeluaran (Penyaluran)</h2>
    
    <form action="{{ route('finance.disbursements.store') }}" method="POST">
        @csrf
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <input type="date" name="date" class="w-full rounded-lg border-gray-300 focus:ring-red-500 focus:border-red-500 shadow-sm" required>
            </div>

            <!-- Penerima Manfaat -->
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Penerima Manfaat</label>
                <input type="text" name="recipient" class="w-full rounded-lg border-gray-300 focus:ring-red-500 focus:border-red-500 shadow-sm" placeholder="Nama Penerima / Vendor" required>
            </div>
            ...
            <div>
                ...
                <select name="bank_sender" id="bank_sender" class="w-full rounded-lg border-gray-300 focus:ring-red-500 focus:border-red-500 shadow-sm">
                    <option value="BSI">BSI Main Account</option>
            ...
            <div id="bank_sender_custom_group" style="display: none;">
                <label class="block text-sm font-medium text-gray-700 mb-1">Nama Bank Lainnya</label>
                <input type="text" name="bank_sender_custom" class="w-full rounded-lg border-gray-300 focus:ring-red-500 focus:border-red-500 shadow-sm" placeholder="Masukkan nama bank">
            </div>

            <!-- Uang Keluar -->
            <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Jumlah Keluar (Rp)</label>
                <input type="number" name="amount" class="w-full rounded-lg border-gray-300 focus:ring-red-500 focus:border-red-500 shadow-sm" placeholder="0" required>
            </div>

            <!-- Biaya Admin -->
             <div>
                <label class="block text-sm font-medium text-gray-700 mb-1">Biaya Admin (Rp)</label>
                <input type="number" name="admin_fee" class="w-full rounded-lg border-gray-300 focus:ring-red-500 focus:border-red-500 shadow-sm" value="0">
            </div>
            ...
            <div class="md:col-span-2">
                ...
                <div class="flex gap-4 p-3 bg-gray-50 rounded-lg border border-gray-200">
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="type" value="project" checked class="text-red-600 focus:ring-red-500">
                        <span class="text-sm text-gray-700">Penyaluran Proyek (Potong Dana Proyek)</span>
                    </label>
                    <label class="flex items-center gap-2 cursor-pointer">
                        <input type="radio" name="type" value="amil" class="text-red-600 focus:ring-red-500">
                ...
                <select name="project_id" id="projectSelect" class="w-full rounded-lg border-gray-300 focus:ring-red-500 focus:border-red-500 shadow-sm" required>
                    ...
                    @foreach($projects as $project)
                         <option value="{{ $project->id }}" 
                                 ...
                                 data-max-expense="{{ $project->balance }}">
            ...
            <script>
                document.addEventListener('DOMContentLoaded', function() {
                    ...
                    function filterProjects() {
                        ...
                        // Sembunyikan filter program/pilar jika tipe Amil
                        ...
                        projectOptions.forEach(option => {
                            ...
                            if (selectedType === 'amil') {
                                // Jika tipe Operasional Amil, CUMA tampilkan proyek SPAI
                                isVisible = isSpaiProject;
                            } else {
                                // Jika tipe Penyaluran Proyek, tampilkan SEMUA kecuali SPAI (dengan filter pilar/program)
                            ...
                            if (isVisible) {
                                ...
                                // Auto-select jika cuma ada 1 yang visible (khusus Amil)
                                if (selectedType === 'amil' && isSpaiProject) {
            ...
            <!-- Keterangan -->
             <div class="md:col-span-2">
                ...
                <textarea name="description" rows="3" class="w-full rounded-lg border-gray-300 focus:ring-red-500 focus:border-red-500 shadow-sm"></textarea>
            </div>
        ...
        <div class="mt-8 flex justify-end gap-3">
            ...
            <button type="submit" class="px-6 py-2 bg-red-600 text-white rounded-lg hover:bg-red-700 transition shadow-lg">Save Disbursement</button>
        </div>
    </form>
</div>
@endsection
